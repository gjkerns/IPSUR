...
postscript(file="ps/Between-versus-within.ps")
  y1 <- rnorm(300, mean = c(2,8,22))
  plot(y1, xlim = c(-1,25), ylim = c(0,0.45) , type = "n")
  f <- function(x){dnorm(x, mean = 2)}
  curve(f, from = -1, to = 5, add = TRUE, lwd = 2)
...
svg(file="svg/Between-versus-within.svg")
  y1 <- rnorm(300, mean = c(2,8,22))
  plot(y1, xlim = c(-1,25), ylim = c(0,0.45) , type = "n")
  f <- function(x){dnorm(x, mean = 2)}
  curve(f, from = -1, to = 5, add = TRUE, lwd = 2)
...
postscript(file="ps/Some-F-plots-HH.ps")
  library(HH)
  old.omd <- par(omd = c(.05,.88, .05,1))
  F.setup(df1 = 5, df2 = 30)
  F.curve(df1 = 5, df2 = 30, col='blue')
...
svg(file="svg/Some-F-plots-HH.svg")
  library(HH)
  old.omd <- par(omd = c(.05,.88, .05,1))
  F.setup(df1 = 5, df2 = 30)
  F.curve(df1 = 5, df2 = 30, col='blue')
...
postscript(file="ps/power-examp.ps")
  library(TeachingDemos)
  power.examp()
dev.off()

svg(file="svg/power-examp.svg")
  library(TeachingDemos)
  power.examp()
dev.off()
