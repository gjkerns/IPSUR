...
#+BABEL: :session *R* :exports results :results value raw :cache no :tangle no
#+LaTeX_CLASS: scrbook
...
Given information about the relationship between \(x\) and \(Y\), we would like to /predict/ future values of \(Y\) for particular values of \(x\). This turns out to be a difficult problem\footnote{Yogi Berra once said, ``It is always difficult to make predictions, especially about the future.''}, so instead we first tackle an easier problem: we estimate \( \E Y \). How can we accomplish this? Well, we know that \(Y\) depends somehow on \(x\), so it stands to reason that
\begin{equation}
...
#+begin_src R :exports none :results graphics silent :file img/philosophy.png
# open window
...
#+begin_src R :exports code :results graphics silent :file img/carscatter.png
...
The parameter estimates \( b_{0} \) and \( b_{1} \) for the intercept and slope, respectively, are shown above. The regression line is thus given by \( \hat{\mu}(\mathtt{speed}) = SRC_R{round(summary(cars.lm)$coef[1], 2)} + SRC_R{round(summary(cars.lm)$coef[2], 2)} \cdot \mathtt{speed} \).
...
#+begin_latex
\begin{figure}[ht!]
  \includegraphics[width=5in, height=4in]{img/carline.png}
...
#+begin_src R :exports code :results output graphics silent :file img/carline.png
...
We may use the regression line to obtain a point estimate of the mean stopping distance for a car traveling 8 mph: \( \hat{\mu}(15) = b_{0} + (8) (b_{1}) \approx SRC_R{round(summary(cars.lm)$coef[1], 2)} + (8) ( SRC_R{round(summary(cars.lm)$coef[2], 2)} ) \approx 13.88 \). We would also use 13.88 as a point estimate for the stopping distance of a future car traveling 8 mph. 
#+latex: \end{example}
...
S^{2}=\frac{SSE}{n-2},
\end{equation}
and we estimate \(\sigma\) with the /standard error/\footnote{Be careful not to confuse the mean square error \(S^{2}\) with the sample variance \(S^{2}\) in Chapter \ref{cha:Describing-Data-Distributions}. Other notation the reader may encounter is the lowercase \(s^{2}\) or the bulky \(MSE\).} \(S=\sqrt{S^{2}}\).
...
#+begin_latex
\begin{figure}[ht!]
  \includegraphics[width=5in, height=4in]{img/carscipi.png}
...
#+begin_src R :exports none :results graphics silent :file img/carscipi.png
library(HH)
...
Here we see that the \(F\) statistic is \( SRC_R{round(as.numeric(carsumry$fstatistic[1]), 2)} \) with a \(p\)-value very close to zero. The conclusion: there is very strong evidence that \(H_{0}:\beta_{1} = 0 \) is false, that is, there is strong evidence that \( \beta_{1} \neq 0 \). Moreover, we conclude that the regression relationship between =dist= and =speed= is significant.
...
#+begin_src R :exports code :results graphics silent :file img/Normal-q-q-plot-cars.png
...
#+begin_src R :exports code :results graphics silent :file img/std-resids-fitted-cars.png
...
#+begin_src R :exports code :results graphics silent :file img/resids-fitted-cars.png
...
#+begin_src R :exports code :results graphics silent :file img/Cooks-distance-cars.png
...
#+begin_src R :exports none :results graphics silent :file img/Diagnostic-plots-cars.png
par(mfrow = c(2,2))
plot(cars.lm)
par(mfrow = c(1,1))
#+end_src
