...
#+results[1904540257c7ba07ed2c9b3ee4b2e17be891234a]:
...
#+results[3ecb16cbe6c84fb3eff9e7f8fc967912e8dc5547]:
#+begin_example
1 | 2: represents 1.2
 leaf unit: 0.1
            n: 272
...
1. For these data, SRC_R[:exports results :results value raw]{names(table(race))[which(table(race)==max(table(race)))]} Caucasian has the highest frequency.

1. For these data, SRC_R[:exports results :results value raw]{names(table(race))[which(table(race)==min(table(race)))]} Other has the lowest frequency.
...
We can answer this by looking above. For these data, the gender with the highest mean salary is SRC_R[:exports results :results value raw]{ names(x)[ which( x==max(x) ) ] } Male.
...
For these data, the the largest standard deviation is approximately SRC_R[:exports results :results value raw]{round(y[which(y==max(y))],2)} 158.54 which was attained by the SRC_R[:exports results :results value raw]{names(y)[which(y==max(y))]} Male gender.
...
Answers will vary. There should be some remarks that the center of the box is farther to the right for the SRC_R[:exports results :results value raw]{names(x)[which(x==max(x))]} Male gender, and some recognition that the box is wider for the SRC_R[:exports results :results value raw]{names(y)[which(y==max(y))]} Male gender.
...
#+results[91b09e5eef04e8a3e8a0552e9fd44c4fee21b8f2]:
...
#+results[bf0504a5e7e5a63aa33823e1270ee049d07dc772]:
: [1] 419
: [1] 256.75
: [1]    3.0   78.5  139.5  336.0 1632.0
: [1] 257.5

#+latex: \noindent 
Compare your answers (3) and (5). Are they the same? If not, are they close?

Yes, they are close, within SRC_R[:exports results :results value raw]{ abs(IQR(x)-(fivenum(x)[4] - fivenum(x)[2])) } 0.75 of each other.
...
#+results[6f8b3d58680d625fb6c46079c1f7402dc9bfd8ad]:
: [1] 166 167 168
: [1] 168

Observations SRC_R[:exports results :results value raw]{which(x > inF)}
| 166 |
| 167 |
| 168 |
 would be considered potential outliers, while observation(s) SRC_R[:exports results :results value raw]{which(x > outF)} 168 would be considered a suspected outlier.
...
We should take the sample skewness value and compare it to $2\sqrt{6/n}\approx$ SRC_R[:exports results :results value raw]{ round( 2*sqrt(6 / length(before)), 3 ) } 0.378 in absolute value to see if it is substantially different from zero. The direction of skewness is decided by the sign (positive or negative) of the skewness value. 


We should take the sample kurtosis value and compare it to $2\cdot\sqrt{24/168}\approx$ SRC_R[:exports results :results value raw]{round(4*sqrt(6/length(before)),3)} 0.756, in absolute value to see if the excess kurtosis is substantially different from zero. And take a look at the sign to see whether the distribution is platykurtic or leptokurtic.
...
We should do for this one just like we did previously. We would again compare the sample skewness and kurtosis values (in absolute value) to SRC_R[:exports results :results value raw]{round(2*sqrt(6/length(after)),3)} 0.378 and SRC_R[:exports results :results value raw]{round(4*sqrt(6/length(after)),3)} 0.756, respectively.
