...
#+BABEL: :session *R* :results output raw :tangle no 
#+LaTeX_CLASS: scrbook
...
#+begin_src R :exports code :results graphics :file img/stripcharts.pdf
...
#+begin_src R :exports none :results graphics :file img/indpl-lakehuron.pdf
...
#+begin_src R :exports none :results graphics :file img/bar-gr-stateregion.pdf
par(mfrow = c(1,2)) # 2 plots: 1 row, 2 columns
barplot(table(state.region), cex.names = 0.50)
barplot(prop.table(table(state.region)), cex.names = 0.50)
par(mfrow = c(1,1)) # back to normal
...
#+begin_src latex 
  \begin{figure}[ht!]
    ...
    \caption[Bar graphs of the \texttt{state.region} data]{\small The left graph is a frequency barplot made with =table= and the right is a relative frequency barplot made with =prop.table=.}
...
#+begin_src R :exports code :results graphics :file img/Pareto-chart.pdf
library(qcc)
...
#+begin_src R :exports code :results graphics :file img/dot-charts.pdf
...
#+begin_src R :exports none :results graphics :file img/xyplot-group.pdf
library(lattice)
...
#+begin_src R :exports none :results graphics :file img/bwplot.pdf
library(lattice)
...
#+begin_src R :exports none :results graphics :file img/histograms-lattice.pdf
library(lattice)
...
#+begin_src R :exports none :results graphics :file img/xyplot.pdf
...
#+begin_src R :exports none :results graphics :file img/coplot.pdf
...
1. For these data, SRC_R{names(table(race))[which(table(race)==max(table(race)))]} has the highest frequency.

1. For these data, SRC_R{names(table(race))[which(table(race)==min(table(race)))]} has the lowest frequency.

1. The graph is shown below.


#+begin_src R :exports code :results graphics :file img/barplotRTDrace.pdf
...
#+begin_src latex 
  \begin{figure}[ht!]
    \centering
    \includegraphics[width=5in, height=4in]{img/barplotRTDrace.pdf}
...
We can answer this by looking above. For these data, the gender with the highest mean salary is SRC_R{ names(x)[ which( x==max(x) ) ] }.
...
For these data, the the largest standard deviation is approximately SRC_R{round(y[which(y==max(y))],2)} which was attained by the SRC_R{names(y)[which(y==max(y))]} gender.

1. Make boxplots of =salary= by =gender=. How does the boxplot compare to your answers to (1) and (3)?

The graph is shown below.

#+begin_src R :exports code :results graphics :file img/boxplotRTDsalgen.pdf
...
Answers will vary. There should be some remarks that the center of the box is farther to the right for the SRC_R{names(x)[which(x==max(x))]} gender, and some recognition that the box is wider for the SRC_R{names(y)[which(y==max(y))]} gender.
...
Yes, they are close, within SRC_R{ abs(IQR(x)-(fivenum(x)[4] - fivenum(x)[2])) } of each other.

\noindent The boxplot of =reduction= is below.

#+begin_src R :exports code :results graphics :file img/boxplotRTDreduc.pdf
...
Observations SRC_R[:results raw]{which(x > inF)} would be considered potential outliers, while observation(s) SRC_R{which(x > outF)} would be considered a suspected outlier.
...
#+begin_src R :exports code :results graphics :file img/boxplotRTDbefore.pdf
...
#+begin_src latex 
  \begin{figure}[ht!]
    \centering
    \includegraphics[width=5in, height=4in]{img/boxplotRTDbefore.pdf}
...
#+begin_src R :exports code :results graphics :file img/boxplotRTDafter.pdf
...
We should take the sample skewness value and compare it to $2\sqrt{6/n}\approx$ SRC_R{ round( 2*sqrt(6 / length(before)), 3 ) } in absolute value to see if it is substantially different from zero. The direction of skewness is decided by the sign (positive or negative) of the skewness value. 


We should take the sample kurtosis value and compare it to $2\cdot\sqrt{24/168}\approx$ SRC_R{round(4*sqrt(6/length(before)),3)}), in absolute value to see if the excess kurtosis is substantially different from zero. And take a look at the sign to see whether the distribution is platykurtic or leptokurtic.
...
We should do for this one just like we did previously. We would again compare the sample skewness and kurtosis values (in absolute value) to SRC_R{round(2*sqrt(6/length(after)),3)} and SRC_R{round(4*sqrt(6/length(after)),3)}, respectively.
...
#+begin_src R :exports code :results graphics :file img/histRTDbefore.pdf
...
#+begin_src latex 
  \begin{figure}[ht!]
    \centering
    \includegraphics[width=5in, height=4in]{img/histRTDbefore.pdf}
...
#+begin_src R :exports code :results graphics :file img/histRTDafter.pdf
...
#+begin_src latex 
  \begin{figure}[ht!]
    \centering
    \includegraphics[width=5in, height=4in]{img/histRTDafter.pdf}
...
#+begin_src R :exports none :results graphics :file img/diagram.pdf
require(diagram)
par(mex = 0.2, cex = 0.5)
...
#+latex: \paragraph*{The \textt{isin} function}
...
#+begin_src R :exports none :results graphics :file img/birthday.pdf
